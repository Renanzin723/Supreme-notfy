...
const BrandUtmify = () => {
  useEffect(() => {
    ...
    const metaTags = [
      { name: 'apple-mobile-web-app-title', content: 'Utmify' },
    ...
    appleTouchIcon.setAttribute('href', '/icons/utmify/icon-180.png?v=4');
    appleTouchIcon.setAttribute('data-brand-link', 'true');
    document.head.appendChild(appleTouchIcon);

    const manifestLink = document.createElement('link');
    manifestLink.setAttribute('rel', 'manifest');
    manifestLink.setAttribute('href', '/manifests/utmify.webmanifest');
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-600 to-blue-800 flex items-center justify-center p-4">
      <div className="max-w-md mx-auto text-center text-white">
        <div className="w-32 h-32 mx-auto mb-6 rounded-3xl bg-white/10 backdrop-blur flex items-center justify-center shadow-2xl">
          <BankLogo bank="utmify" size={96} />
        </div>
        
        <h1 className="text-4xl font-bold mb-4">Utmify</h1>
      ...
      <script dangerouslySetInnerHTML={{__html: `
        (function() {
          var BRAND = { slug: 'utmify', name: 'Utmify' };
          try {
