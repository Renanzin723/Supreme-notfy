...
const BrandSantander = () => {
  useEffect(() => {
    document.title = 'Santander';
    ...
    const metaTags = [
      { name: 'apple-mobile-web-app-title', content: 'Santander' },
    ...
    appleTouchIcon.setAttribute('href', '/icons/santander/icon-180.png?v=4');
    appleTouchIcon.setAttribute('data-brand-link', 'true');
    document.head.appendChild(appleTouchIcon);

    const manifestLink = document.createElement('link');
    manifestLink.setAttribute('rel', 'manifest');
    manifestLink.setAttribute('href', '/manifests/santander.webmanifest');
    ...
    <div className="min-h-screen bg-gradient-to-br from-red-600 to-red-800 flex items-center justify-center p-4">
      <div className="max-w-md mx-auto text-center text-white">
        <div className="w-32 h-32 mx-auto mb-6 rounded-3xl bg-white/10 backdrop-blur flex items-center justify-center shadow-2xl">
          <BankLogo bank="santander" size={96} />
        </div>
        
        <h1 className="text-4xl font-bold mb-4">Santander</h1>
      ...
      <script dangerouslySetInnerHTML={{__html: `
        (function() {
          var BRAND = { slug: 'santander', name: 'Santander' };
          try {
